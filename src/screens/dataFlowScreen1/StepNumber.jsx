...
const getOperatorFromPrefix = (prefix) => {
  return OPERATOR_NAMES[prefix] || null;
};
  ...
  const handleNumberChange = (input) => {
    ...
    if (!validation.isValid && limitedInput.length > 0) {
      ...
      if (limitedInput.startsWith('75') || !limitedInput.startsWith('7')) {
        onChange("");
        onPrefixChange && onPrefixChange(null);
        setCurrentPrefix(null);
        setOperatorName(null);
        return;
      }
    ...
    if (limitedInput.length >= 2) {
      const prefix = limitedInput.substring(0, 2);
      onPrefixChange && onPrefixChange(prefix);
      setCurrentPrefix(prefix);
      setOperatorName(getOperatorFromPrefix(prefix));
    } else {
      onPrefixChange && onPrefixChange(null);
      setCurrentPrefix(null);
      setOperatorName(null);
